...
from directionality_analysis import get_directionality_files_list, directionality_files_to_df, create_seizure_uuid, replace_multiple_outliers
...
def SDSpeedsCellAlgorithm(df_onsets_input, i_wave, n_neighbors=1, plot_res=False, vectorize=False, onset_sz=False):
    ...
    for i_group, session_group in df_onsets_input[df_onsets_input[onset_type].notna()].groupby("uuid_extended"):
        ...
        nearest_indices_later_onset = np.argsort(
            later_neurons_distances, axis=1)[:, :n_neighbors]
            ...
            if np.isinf(later_neurons_distances[i_neuron]).all():
                ...
                if isinstance(neuron_nearest_indices, Iterable):
                    ...
                    for i_neighbor, index_neighbor in enumerate(neuron_nearest_indices):
                        ...
                        dt = (x_y_onset[index_neighbor][2] -
                              x_y_onset[i_neuron][2]) / 15.0
                        v_neighbor = ds/dt
                    ...
                    # [frames] / ([frames]/[second])
                    dt = (x_y_onset[neuron_nearest_indices[0]]
                          [2] - x_y_onset[i_neuron][2]) / 15.0
                    vs[i_neuron] = ds/dt
        vs_2d.append(vs)
...
def main(folder: str, save_data: bool, save_figs: bool = False, file_format: str = "pdf", n_neighbors: int = 1):
    ...
    dict_uuid_exp_type = {uuid: dd.getExperimentTypeForUuid(
        uuid) for uuid in df_onsets.uuid.unique()}
    ...
    uuids_neuron1, ids_neuron1, vs_neuron1 = SDSpeedsCellAlgorithm(
        df_onsets, 1, n_neighbors)
    uuids_neuron2, ids_neuron2, vs_neuron2 = SDSpeedsCellAlgorithm(
        df_onsets, 2, n_neighbors)
    uuids_neuron_sz, ids_neuron_sz, vs_neuron_sz = SDSpeedsCellAlgorithm(
        df_onsets, 2, n_neighbors, False, False, True)
    # flatten all arrays
    vs_neuron_sz_flat = [
        element for sublist in vs_neuron_sz for element in sublist]
    uuids_neuron_sz_flat = [uuids_neuron_sz[i] for i, neurons in enumerate(
        vs_neuron_sz) for j in range(len(neurons))]
    vs_neuron1_flat = [
        element for sublist in vs_neuron1 for element in sublist]
    uuids_neuron1_flat = [uuids_neuron1[i] for i, neurons in enumerate(
        vs_neuron1) for j in range(len(neurons))]
    vs_neuron2_flat = [
        element for sublist in vs_neuron2 for element in sublist]
    uuids_neuron2_flat = [uuids_neuron2[i] for i, neurons in enumerate(
        vs_neuron2) for j in range(len(neurons))]
    ...
    vs_df["mouse_id"] = vs_df.apply(lambda row: dd.getMouseIdForUuid(
        extended_to_normal_uuid(row["uuid"])), axis=1)
    vs_df_sz["mouse_id"] = vs_df_sz.apply(lambda row: dd.getMouseIdForUuid(
        extended_to_normal_uuid(row["uuid"])), axis=1)
    ...
    means_per_session = vs_df.groupby(["exp_type", "mouse_id", "uuid", "i_wave"]).median(
    ).reset_index()  # mean().reset_index()
    means_per_session["speed_type"] = "SD"
    sz_means_per_session = vs_df_sz_outliers_removed.groupby(["exp_type", "mouse_id", "uuid"]).median(
    ).reset_index()  # vs_df_sz.groupby(["exp_type", "mouse_id", "uuid"]).mean().reset_index()
    ...
    df_mean_speeds = pd.concat([means_per_session[["exp_type", "mouse_id", "uuid", "speed_type", "i_wave", "v_umps", "v_mmpmin"]],
                                sz_means_per_session[["exp_type", "mouse_id", "uuid", "speed_type", "i_wave", "v_umps", "v_mmpmin"]]]).reset_index(drop=True)
    if save_data:
...
if __name__ == "__main__":
    ...
    main(args.folder, args.save_data, args.save_figs,
         args.file_format, args.n_neighbors)
