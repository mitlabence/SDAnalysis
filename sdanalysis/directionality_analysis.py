...
def directionality_files_to_df(
    ...
    all_onsets_df["mouse_id"] = df_id_uuid[
        df_id_uuid["uuid"] == uuid_dict[files_list[0]]
    ]["mouse_id"].values[0]
    ...
    for fpath in files_list[1:]:
        ...
        df["mouse_id"] = df_id_uuid[df_id_uuid["uuid"] == uuid_dict[fpath]][
            "mouse_id"
        ].values[0]
...
def replace_outliers(
    ...
    deviation_threshold = deviations_nonan_sorted_desc[
        ceil(percent * len(deviations_nonan_sorted_desc))
    ]
...
def replace_multiple_outliers(
    df: pd.DataFrame,
    colnames: str = "onset_sz",
    percent: float = 0.05,
    replace_value=np.NaN,
) -> pd.DataFrame:
...
def get_quantiles(df_onsets: pd.DataFrame) -> pd.DataFrame:
    ...
    quantile_dfs = []
    for event_type, quantile_name in zip(
        ["sd1", "sd2", "sz"], ["quantile1", "quantile2", "quantile_sz"]
    ):
...
def get_angles(df_quantiles: pd.DataFrame, drop_na: bool = False) -> pd.DataFrame:
    ...
    for uuid, group in session_groups:
        ...
        if len(sz_row) == 0:  # no seizure for the session
            ...
            if len(sd1_row["dx"].values) > 0 and len(sd2_row["dx"].values) > 0:
                sd1_dot_sd2 = (
                    sd1_row["dx"].values[0] * sd2_row["dx"].values[0]
            ...
            if len(sz_row["dx"].values) > 0 and len(sd1_row["dx"].values) > 0:
                sz_dot_sd1 = (
                    sz_row["dx"].values[0] * sd1_row["dx"].values[0]
            ...
            if len(sd2_row["dx"].values) > 0 and len(sd1_row["dx"].values) > 0:
                sz_dot_sd2 = (
                    sz_row["dx"].values[0] * sd2_row["dx"].values[0]
    ...
    df_angles["angle_deg"] = df_angles["angle"].apply(lambda x: x * 180.0 / pi)
    ...
    df_angles["event_index"] = (
        df_angles.groupby("uuid_extended", sort=False).ngroup() + 1
    )
...
def main(
    ...
    df_quantiles["y_mirrored"] = df_quantiles.apply(lambda row: -1 * row["y"], axis=1)
    # add dx and dy columns
    df_quantiles = df_quantiles.groupby(
        ["uuid_extended", "quantile_type"], as_index=False
    ).apply(
        lambda group_df: group_df.assign(
            dx=lambda gdf: get_dx_first_last_quantile(gdf, "x"),
            dy=lambda gdf: get_dx_first_last_quantile(gdf, "y_mirrored"),
        )
    )
    ...
    df_angles_aggregate = (
        ...
        .sort_values(by="mouse ID")
        .groupby(["mouse ID", "angle_type"])
        .apply(lambda g: g["angle_deg"].mean())
    )
    ...
    if save_data:
        ...
        df_angles_aggregate.to_excel(
            os.path.join(
                output_folder,
                f"directionality_aggregate_{dataset_type}_{output_dtime}.xlsx",
            ),
            index=False,
        )
